...
def test_a_created_user_can_actually_sign_in(admin_client, db, identity):
    ...
    # ...but nothing else works until the password is theirs
    assert newcomer.get("/api/airsizer/config").status_code == 403
    assert newcomer.get("/api/airsizer/config").json()["detail"] == "Password change required."
    ...
    # now the product answers
    assert newcomer.get("/api/airsizer/config").status_code == 200
...
def test_removing_a_seat_takes_the_product_away_at_once(admin_client, db, identity):
    from backend.main import app
    engineer = db_user(db, ENGINEER_EMAIL)
    client = TestClient(app)
    ...
    admin_client.delete(f"{USERS}/{engineer.id}/licenses/engineering",
                        headers=headers(admin_client))
    assert client.get("/api/airsizer/config").status_code == 403
...
def test_suspending_someone_cuts_them_off_on_the_next_call(admin_client, db, identity):
    ...
    admin_client.patch(f"{USERS}/{engineer.id}", headers=headers(admin_client),
                       json={"status": "suspended"})
    assert client.get("/api/airsizer/config").status_code == 401
